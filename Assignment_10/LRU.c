...
int find_victim_page(int proc_id) {
    ...
    for (int i = ESSENTIAL_SEGMENT_PAGES; i < VIRTUAL_PAGES_PER_PROCESS; i++) {
        if (process_page_tables[proc_id][i].valid_ref & (1 << 15)) {  // Valid bit check
...
int main() {
    ...
    while (active_processes > 0) {
        for (int i = 0; i < total_processes && active_processes > 0; i++) {
            if (system_processes[i].current_search_number >= total_searches) continue;
            #ifdef VERBOSE
...
#ifdef VERBOSE
            // printf("\tSearch %d by Process %d\n", system_processes[i].current_search_number + 1, i);
#endif
    ...
    printf("Total     %d       %d   (%.2f%%)     %d (%.2f%%)         %d + %d + %d + %d (%.2f%% + %.2f%% + %.2f%% + %.2f%%)\n",
           total_page_accesses, total_page_faults, total_fault_percent,
           total_replacements, total_replace_percent,
           attempt_counts[0], attempt_counts[1], attempt_counts[2], attempt_counts[3],
           total_a1, total_a2, total_a3, total_a4);
